...
export default function Home(){
 ...
 return <main>
  ...
  <section className="wrap downloads" id="downloads"><div className="sectionHead"><div><p className="eyebrow dark">ANEXOS, MANUAIS E FONTES</p><h2>Documentação para consulta</h2></div><p>Inclui relatório, apresentação, Manual de Utilização, plano de capacitação e fontes públicas sem identificadores empresariais.</p></div><div className="downloadGrid">{DOWNLOADS.map(([title,type,file])=><a key={file} href={'/downloads/'+file} download><span>{type}</span><h3>{title}</h3><b>Descarregar ↓</b></a>)}</div></section>
